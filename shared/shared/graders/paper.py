"""Paper-identity retrieval grader.

Scores search results against a single gold paper described by
``{doi, title}``. Each result is reduced to candidate paper identifiers and
matched to the gold paper in priority order:
...
agents that return arbitrary URLs — comparable apples-to-apples on paper
...
class PaperRetrievalGrader:
    """Deterministic paper-identity matcher for a single search result.

    Returns ``is_match = 1.0`` when the result identifies the gold paper by
    ...
    def match(self, result: SearchResult, gold_paper: dict) -> tuple[bool, str | None]:
        """Return ``(matched, method)`` for one result against the gold paper."""
